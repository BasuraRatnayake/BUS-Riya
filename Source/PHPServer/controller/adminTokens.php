...
class AdminTokens {    
    ...
    public function createAuthToken($username, $refreshToken = null){//Create Token from TimeStamp   
        ...
        if($refreshToken == null){        
            if($this->database->numberOfRecords("username", "tbl_tokens_admin", "WHERE username = '$username'") > 0){
                $this->database->delete("tbl_tokens_admin", "WHERE username = '$username'");
            }
            ...
            if($this->database->insert("tbl_tokens_admin", "'$token', '$username', '$timeStamp', '$refreshToken'")){
                $data = array(
                    "status" => true,
                    "response_code" => "200",
            ...
            if($this->database->numberOfRecords("username", "tbl_tokens_admin", "WHERE username = '$username' AND refreshToken = '$token'") > 0){                 
                $this->database->delete("tbl_tokens_admin", "WHERE username = '$username'");               
                ...
                if($this->database->insert("tbl_tokens_admin", "'$username', '$timeStamp', '$token', '$hashSalt', '$refreshToken'")){
                    $data = array(
                        "status" => true,
                        "response_code" => "200",
                        "message" => "Authentication Success",
    ...
    public function validateAuthToken($token){//Validate Token
        ...
        if($this->database->numberOfRecords("username", "tbl_tokens_admin", "WHERE token = '$token'") == 1){
            $dataDB = $this->database->fetchFullRow("timeCreated, token, username, refreshToken", "tbl_tokens_admin", "WHERE token = '$token'");
            ...
            if($timeStamp[1] == "sec" || $timeStamp[1] == "min"){
                if($timeStamp[0] <= 30){
                    ...
                    if($timeStamp[1] == "sec"){
                        ...
                        $this->database->delete("tbl_tokens_admin", "WHERE token = '$token'");
                        $data = array(
                            "status" => false,
                            "response_code" => "410",
